...
lucide.createIcons();
    ...
    document.querySelectorAll('.mv-expandible').forEach(function(card) {
      card.addEventListener('click', function(e) {
        // No cerrar si el click fue en un botón interno
  ...
  // Función global para cerrar todas las tarjetas mv (usada en botones internos)
  window.cerrarTodosMV = function() {
    document.querySelectorAll('.mv-expandible').forEach(function(c) {
      c.classList.remove('mv-card--abierta');
    });
  };

  // ── Fondo dinámico hero-admin desde data-foto ──────────────────
  const heroAdmin = document.getElementById('heroAdmin');
  if (heroAdmin) {
    const foto = heroAdmin.getAttribute('data-foto');
    if (foto) {
      heroAdmin.style.backgroundImage = `url('${foto}')`;
      heroAdmin.style.backgroundSize = 'cover';
      heroAdmin.style.backgroundPosition = 'center';
    }
  }